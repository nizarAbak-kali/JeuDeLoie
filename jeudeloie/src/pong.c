...
int jouer_pong(SDL_Surface* screen,int _indicej1 , int _indicej2){
  ...
  //  *screen
  //creation des positions de ces surfaces
  ...
  screen = SDL_SetVideoMode(TAILLE_FENETRE_X,TAILLE_FENETRE_Y, 32, SDL_HWSURFACE | SDL_DOUBLEBUF | SDL_RESIZABLE);
  SDL_WM_SetCaption("PONG  SDL", NULL);
  */
 ...
 balise_restart:
  ...
  while (continuer)
    {
      SDL_PollEvent(&event);
      // gestion des controles des raquettes
      switch(event.type) {
        case SDL_QUIT:
	  continuer = 0;
	  break;
