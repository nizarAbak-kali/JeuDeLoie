 ...
 * \date 11 mars 2013
 *
 * Fichier de lancement d'une partie.
 */
...
/*Fonction qui permet de saisir un nom du fichier au clavier tout en affichant la saisi sur l'écran principal en utilisant SDL_ttf*/
void nom_Fichier(SDL_Surface *ecran, char nom[]) {
  //Déclaration des variables nécessaires pour le bon foncionnement de la procédure
  ...
  //Initialisation du nom  avec des 'underscore' 
  for(i=0;i<10;i++)
    nom[i]='_';
  nom[i]='\0';

  //Chargement de l'image du fond et Initialisation du texte de demande de la saisie
  ...
  //Initialisation des position du texte et de l'image du fond
  postexte.x=(ecran->w / 2)-(texte->w /2);
  postexte.y=50;
  posnom.x=(ecran->w /2 )-(_nom->w /2);
  posnom.y=250;

  //Nettoyage de l'écran blittage des surfaces (Image de fond et texte)  
  ...
  while(continuer) {
    ...
    case SDL_KEYDOWN:
	  ...
	  break;
	  //Récupération des caractères saisis au clavier	    
	  default:c=event.key.keysym.sym;break;
      }	    

      //Si le bouton sur lequel on a appuyé correspond à une lettre
      if((k < 10  && c >= 'a'  && c<='z')) {
	nom[k]=c;
	...
	k++;
      ...
      //Si le bouton sur lequel on a appuyé correspond à un backspace
      if(k > 0 && c == '\b') {
	nom[k-1]='_';
	_nom=TTF_RenderText_Blended(font2,nom,couleur);
	k--;
    ...
    default:break;
      ...
      /*****  Raffraichissement de l'écran après saisie des lettres  *****/
      SDL_BlitSurface(fond,NULL,ecran,NULL);
      SDL_BlitSurface(texte,NULL,ecran,&postexte);
      SDL_BlitSurface(_nom,NULL,ecran,&posnom);
      SDL_Flip(ecran);
  ...
  //Supression des underscores restant à la fin du nom
  for(i = 9; nom[i] == '_'; i--);
  nom[i+1] = '\0';
...
/* Fonction qui permet de sauvegarder une partie dans un fichier txt. (Elle sauvegarde 5 parties au maximum)  */
int sauvegarder_Partie(SDL_Surface *ecran, char nom_fichier[], Joueur joueurs[], int num_carte, int nb_joueurs) {
  //Déclaration des variables locales nécessaires
  FILE *fichier = NULL, *nb_parties = NULL;
  int i, nb;
  char ch1[10], ch2[10], ch3[10], ch4[10], ch5[10];

  //Chargement du nombre de parties déjà sauvegardées
  nb_parties = fopen("nb_parties.txt","r+");
  fscanf(nb_parties,"%d\n",&nb);
  

  //Chargement des noms des paties déjà sauvegardées (Pour pouvoir les remettre dans le fichier à la fin)
  for(i = 1; i <= nb; i++) {
    switch(i) {
  ...
  //Si nb est inférieur ou égal à 5, valider l'enregistrement de la partie dans le fichier nb_parties.txt
  if(nb <= 5) {
    nb_parties = fopen("nb_parties.txt","w+");
    fprintf(nb_parties,"%d\n",nb);
  }

  //Si nb <= 5 ; création du fichier et sauvegarde de la partie.
  if(nb <= 5) {
    //écriture du nom du nouveau fichier
    ...
    //Ajout du nom du nouveau fichier dans le fichier nb_parties.txt
    fprintf(nb_parties,"%s\n",nom_fichier);
    fclose(nb_parties);
    //Enregistrement des données de la partie dans le fichier correspondant (Le fichier sera créé).
    ...
    fclose(fichier);
    //Retourner 1 quand le fichier est enregistré avec succès
    return 1;
  }
  //Retourner 0 en cas d'erreur ou en cas où y a déjà 5 parties sauvegardées.
  return 0;
}

/* Fonction qui sert de Menu de suspension/arrêt d'une partie */
void menu_Exit(SDL_Surface *ecran, Joueur joueurs[], int num_carte, int nb_joueurs) {
  ...
  //Initialisation des positions des 2 textes qui corresponderont aux 2 options disponibles dans le menu.
  positionoption1.x = 50;
  positionoption1.y = 350;
  positionoption2.x = 50;
  positionoption2.y = 450;
 
  //Boucle while permettant de maintenir l'affichage du Menu Exit
  while(continuer) {
    SDL_WaitEvent(&event);
    ...
    switch(event.type) {
      case SDL_KEYDOWN : 
	    ...
	    //En cas d'appuie sur Entrer
	  case SDLK_RETURN : if(bool == 1) {
	                       //Si partie non sauvegardée (5 Maximum)
				 ...
				 SDL_Flip(ecran);
                               ...
                               //Si partie sauvegardée avec succès
			       else {
    ...
    //Changer la taille de la police de chaque option en fonction de celle qui est sélectionnée.
    if (bool == 1) {
...
/* Fonction qui sert de Menu de Chargement d'une partie sauvegardée */
int menu_Chargement(SDL_Surface *ecran, int *num_cartechoisie, Joueur joueur[], int *nbre_joueur) {
  ...
  //S'il n'y a aucune partie sauvegardée, sauter la boucle while qui sert à afficher les fichiers disponibles (Car il y en a pas)
  if(!nb) continuer = 0;
  ...
  //Initialisation des positions des noms correspondant aux fichiers
  ...
  //Boucle while permettant de choisir un fichier pour le charger.
  while(continuer) {
    SDL_WaitEvent(&event);
    switch(event.type) {
      case SDL_KEYDOWN : 
	  ...
	  case SDLK_RETURN : 
	    //En cas d'appuie sur Entrer, ouvrir le fichier correspondant
	    switch(bool) {
    ...
    //Initialisation des tailles de la police des noms
    ...
    //Agrandir le nom du fichier sélectionné
    switch(bool) {
  ...
  //Si 0 parties sauvegardées, afficher le texte correspondant
  if(!nb) {
    liste = TTF_RenderText_Blended(font2,"0 Parties Sauvegardes",couleurtexte);
    SDL_FillRect(ecran,NULL,SDL_MapRGB(ecran->format,255,255,255));  
    SDL_BlitSurface(chargement,NULL,ecran,&positionchargement);
    SDL_BlitSurface(liste,NULL,ecran,&positionliste);
    SDL_Flip(ecran);
  ...
  //Sinon charger les données de la partie choisie dans le tableau des joueurs, et charger aussi le nom du fichier, le n° de la carte choisie et le nombre des joueurs.
  else {
    char chn[20];
    fscanf(fichier,"%s\n",chn);
    fscanf(fichier,"%d\n",num_cartechoisie);
    ...
    nb_parties = fopen("nb_parties.txt","w+");
    fprintf(nb_parties,"%d\n",nb-1);
    //Cette boucle for permet de supprimer le nom du fichier chargé du fichier nb_parties.txt et de ré-enregistrer les autres (Une partie chargée est supprimée de la liste)
    //P.S : On a quand même la possibilité de la re-sauvegarder sous le même nom ou même sous un nom différent.
    for(i = 1; i <= 5; i++) {
      if(i != bool) {
	switch(i) {
	case 1: fprintf(nb_parties,"%s\n",ch1); break;
...
// affiche le numero des case du plateau
...
/* Fonction qui gere le lancement du dé : son/images/valeur du dé */
int lancement_De(SDL_Surface *ecran, char j[]) {
  ...
  //Chargement des différentes faces du dé, d'une image d'un dé 3d et une surface blanche(car on a qu'une partie de l'écran à utiliser).
  ...
  //Chargement de 2 types de police
  police1 = TTF_OpenFont("../Police/chata.ttf", 50);
  police2 = TTF_OpenFont("../Police/chata.ttf", 15);

  //Initialisation des textes contenant le nom du joueur et l'action à faire.
  ...
  while (continuer) {
    ...
    case SDL_KEYDOWN :
      //S'il s'agit du bouton "Espace", maintenir l'affichage de l'image qui correspond à la valeur obtenue avec le texte qui va avec.
    ...
    //Tant que l'utilisateur n'a pas appuyé sur Espace ou n'a pas fermé la fenêtre, générer un nombre au hasard 'entre 0 et 5'
    if(continuer) {
      srand(time(NULL));
      nb_aleatoire = rand()%6;
    } 
  ...
  //Renvoie de la valeur obtenue +1 (Car elle doit être comprise entre 1 et 6 et pas entre 0 et 5)
...
int gestion_Bonus (SDL_Surface *ecran,TTF_Font *font1,char *nom_joueur){
  ...
  /*  Bonus aleatoire  */
  srand(time(NULL));
  bonus=(rand()%3)+1;//valeur du bonus entre 1 et 3
...
int gestion_Malus(SDL_Surface *ecran,TTF_Font *font1,char *nom_joueur)
{
  ...
  // valeur du malus comprise entre  -1 a -3
  malus =-(rand()%3)-1;
...
void gestion_Mini_Jeux(SDL_Surface *ecran,TTF_Font *font1,Joueur* tabjoueur,int nb_joueur){
 ...
 switch(minijeux){
  ...
  case 1:
    ...
    //Faire avancer le gagnant de 3 cases et reculer le perdant de 3 cases (Idem pour les autres jeux)  
    if(winner == 1){
