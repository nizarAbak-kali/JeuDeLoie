/**
 * \file puissance_4.c
 * \brief Code source du Mini-jeu Puissance 4 en SDL.
...
int Puis_4(int a, int b) {
  ...
  //Vérification veticale vers le haut
  if(a >= 3) {
    for(i = a; i >= a-3; i--) {
  ...
  //Vérification horizontale de droite à gauche.
  noir = orange = 0;
  if(b >= 3) {
    for(i = b; i >= b-3; i--) {
  ...
  //Vérification horizontale de gauche à droite.
  noir = orange = 0;
  if(b <= 3) {
  ...
  //Vérification anti-diagonale vers le haut 
  if(a >= 3 && b <= 3)
    if(((Mat[a][b] == Mat[a-1][b+1]) && (Mat[a-1][b+1] == Mat[a-2][b+2])) && (Mat[a-2][b+2] == Mat[a-3][b+3]))
      return Mat[a][b];

  //Vérification anti-diagonale vers le bas
  if(a <= 2 && b >= 3)
    if(((Mat[a][b] == Mat[a+1][b-1]) && (Mat[a+1][b-1] == Mat[a+2][b-2])) && (Mat[a+2][b-2] == Mat[a+3][b-3]))
      return Mat[a][b];
...
/*Cette fonction parcourt la matice qui contient les valeurs des coups joués et mets à jour la matrice sur l'écran du jeu*/
void update_Screen(SDL_Surface* ecran) {
  ...
  //Parcourt de la matrice des valeurs et blittage des images correspondantes pour chaque case
  for(i = 0; i < NB_LIGNES; i++) {
    for(j = 0; j < NB_COLONNES; j++) {
      switch(Mat[i][j]) {
      case NOIR :
...
/*Fonction principale du jeu de Puissance 4, elle alterne les tours entre 2 joueurs tout en gérant l'affichage des différents évènements de la partie en faisant appel
  aux fonctions créées à cet effet.
  La fonction retourne l'indice du gagnant à la fin de la partie, c'est le seul moyen de la quitter, tant que les joueurs on le même score ils continuerons à jouer*/
int Jouer_Puissance_4(SDL_Surface* ecran, char j1[], char j2[]) {
  ...
  //Chargement de la police du texte "comicbd.ttf"
 ...
 debut:
  ...
  while(continuer) {
	...
	//Si bouton gauche
	  ...
	  //Si position valide et coup jouable, affecter le coup à la matrice des valeurs et alterner le tour
          if(v != -1) {
	    if(Jouable(v)) {
    ...
    //Mise à jour de la matrice sur l'écran du jeu
    update_Screen(ecran);
  ...
  //Si pas de gagnant, relancer le jeu
  if(!verif_Gagnant_p4())
    goto debut;
...
int Accueil_Puissance_4(SDL_Surface *ecran, char *j1, char *j2) {
  ...
  /*Une boucle infine afin de maintenir l'affichage et quitter le programme quand l'utilisateur ferme la fenêtre, ou bien lancer le jeu s'il appuie sur Entrer*/
  while (continuer) {
    SDL_PollEvent(&event);
    switch(event.type) {
    case SDL_QUIT:
    ...
    case SDL_KEYDOWN :
      //S'il s'agit du bouton "Entrer", appeler la fonction Jouer_Puissance_4
