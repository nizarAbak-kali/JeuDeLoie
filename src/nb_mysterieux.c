/**
 * \file nb_mysterieux.c
 * \brief Code source du Mini-jeu Nombre Mystérieux en SDL.
...
//Fonction principale du jeu 'Nombre Mystérieux', elle gérera l'affichage des différentes étapes d'une partie
int Jouer_Nb_Mysterieux(SDL_Surface *ecran, char j1[], char j2[]) {
  ...
  //debut sert de point de départ, il sera appelé lors du changement du joueur.
 debut:
  //Initialisation des différentes variables utilisés dans le jeu et blanchiment de l'écran
  coups_rates = 0;
  ...
  //Boucle while permettant à un joueur de saisir des nombres, il sera quitté une fois le joueur aura trouvé le Nombre Mystérieux
  while(continuer) {
    SDL_WaitEvent(&event);
    switch(event.type) {
      ...
      case SDL_KEYDOWN : 	  
	SDL_FillRect(ecran, NULL, SDL_MapRGB(ecran->format, 55, 151, 208));
	//Vérification des outons appuyés
	  ...
	  case SDLK_RETURN :
	    ...
	    //Si Nombre Mystérieux Inférieur
	    ...
	    //Si Nombre Mystérieux Supérieur
	    else if(coup < nb_mysterieux && coup) {
	      Mix_PlayChannel(1,son,0);
    ...
    //écriture et blittage des différentes surfaces
    sprintf(ch2,"%d",coup);
  ...
  //Si les 2 joueurs ont eu le même score ou si c'est le tour du 2ème joueur, on fait un goto debut
...
/*Cette fonction sert de page d'accueil du jeu Nombre Mystérieux, elle fait appel à la fonction Jouer_Nb_Mysterieux quand l'utilisateur
...
int Accueil_Nb_Mysterieux(SDL_Surface *ecran, char j1[], char j2[]) {
  ...
  while (continuer) {
    ...
    case SDL_KEYDOWN :
      //S'il s'agit du bouton "Entrer", appeler la fonction Jouer_Nb_Mysterieux
