/**
 * \file main.c
 * \brief Le main du projet.
 * \author Matsima Claude
 * \date 11 mars 2013
 *
 * Main du lancement du programme.
 */
 ...
 * \fn int main()
 * \brief Le main du projet.
 * \details C'est la fonction principale du projet qui nous permet d'exécuter les différentes tâches disponibles.
 *
 */
int main (){
		...
		case SDL_KEYDOWN:
		  switch(event.key.keysym.sym)
		    ...
		    case SDLK_RETURN:
		      Mix_PlayChannel(1,son,0);
		      if(Bool == 0){
			jouer(ecran,font1,font2,0);
		        menu=2;	
		      }
		      if(Bool == 1) {
			jouer(ecran,font1,font2,1);
			menu=2;
		      }
		      break;
